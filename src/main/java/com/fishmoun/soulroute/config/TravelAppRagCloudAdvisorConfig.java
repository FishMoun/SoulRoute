...
import com.fishmoun.soulroute.rag.QueryUnderstandingService;
...
class TravelAppRagCloudAdvisorConfig {
    ...
    @Bean
    public DocumentRetriever travelDocumentRetriever(@Qualifier("travelAppVectorStore") VectorStore vectorStore,
                                                     ...
                                                     @Value("${soulroute.rag.query-expansion.enabled:true}") boolean queryExpansionEnabled,
                                                     @Value("${soulroute.rag.query-expansion.max-vector-queries:6}") int maxVectorQueries,
        ...
        return new HybridTravelDocumentRetriever(
                vectorStore,
                jdbcTemplate,
                objectMapper,
                schemaName,
    ...
    @Bean
    public Advisor travelAppRagCloudAdvisor(DocumentRetriever travelDocumentRetriever) {
        return RetrievalAugmentationAdvisor.builder()
                .documentRetriever(travelDocumentRetriever)
                .build();
    }
}
